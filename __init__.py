...
# confirm that the token sent by user matches the one in the server
...
def gconnect():
    ...
    if stored_access_token is not None and gplus_id == stored_gplus_id:
        response = make_response(json.dumps('Current useris' +
                                            'already connected.'),
                                 200)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def newMenuItem(restaurant_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        return "<script>function myFunction() {alert('You are not authorize'" \
        'to create a new menu item. Please create your' \
        "'own menu in order'" \
...
def editMenuItem(restaurant_id, menu_id):
    ...
    restaurant = session.query(Restaurant).filter_by(id=restaurant_id).one()
    if login_session['user_id'] != restaurant.user_id:
        return "<script>function myFunction() {alert('You arenot authorized'" \
        'to edit this menu item. Please create your' \
        "'own menu in order'" \
        "'to edit.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
...
def deleteMenuItem(restaurant_id, menu_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        return "<script>function myFunction() {alert('You arenot authorized'" \
        'to delete this menu item. Please create your' \
        "'own menu in order'" \
